<div class="space-y-2 p-2">
    @php
        ...
        if ($items->isEmpty() && $order->goods && $order->goods->goods_title) {
            $items = collect([$order->goods]);
        }
    @endphp
    @forelse ($items as $item)
        <div class="border rounded-lg p-3 {{ $loop->first ? '' : '' }}">
            <div class="flex items-start justify-between">
                <div>
                    <strong class="text-sm">{{ $item->goods_title ?? $item->product_name }}</strong>
                ...
                @elseif (!empty($item->goods_id))
                    <a href="/product/{{ $item->goods_id }}" target="_blank" class="text-xs text-primary-600 hover:underline">查看商品</a>
                @endif
            </div>
            <div class="grid grid-cols-3 gap-2 mt-2 text-sm">
                ...
                <div>
                    ...
                    <span class="font-medium">NT$ {{ number_format($item->price ?? $item->unit_price) }}</span>
                </div>
                <div>
                    <span class="text-gray-500">小計：</span>
                    <span class="font-medium">NT$ {{ number_format($item->total_price ?? ($item->price * $item->number)) }}</span>
                </div>
            </div>
        </div>
    @empty
