...
class TestController extends Controller
{
    ...
    public function index(){
        ...
        app(AreaService::class)->updateVantTwJson2();;

        /*app(AreaService::class)->updateTwJson();
        dd(12312321);

        $time = date('Y-m-d H:i:s');
        $area = Area::with('son')->where('level',1)->get();
        ...
        foreach($area as $vvv){
            ...
            foreach($vvv->son as $vv){
                if($vv->is_grab == 1){
                    continue;
                }


                //$url = "https://www.post.gov.tw/post/internet/Postal/streetNameData_zip6.jsp?city=".$vvv->name."&cityarea=".$vv->name."&rand=".time();
                $url = "https://www.slimmingtw.com/gettw/";
                ...
                DB::transaction(function () use($vv,$insert) {
                    $vv->is_grab = 1;
                    $vv->save();
                    Area::insert($insert);
                });
    ...
    $city_account = 23;
       ...
       $area = Area::where('parent_id',0)->get();
        ...
        foreach($area as $k=>$item){
            $one = $cityarea_account[$k]+1;
            $two = $cityarea_account[$item->id];
            foreach($cityarea as $kk=>$vv){
                if($kk>=$one && $kk<=$two){
                    $data[$item->id][] = $vv;
                }
